...
from fastapi import FastAPI, Depends, Path, Query, HTTPException, APIRouter , Request
...
def redirect_to_login():
    ...
    redirect_response.delete_cookie("acces_token")#kullanıcının coociesi geçersiz ise siliniyor.
...
async def render_todo_page(request: Request, db: db_dependency):
    try:
        ...
        return templates_.TemplateResponse("todo.html",{"request":request,"todos":todos, "user": user})
    except:
        return redirect_to_login()
...
async def render_add_todo_page(request: Request):
    try:
        ...
        if user is None:
            return redirect_to_login()
        return templates_.TemplateResponse("add-todo.html",{"request":request, "user": user})
    except:
        return redirect_to_login()
...
async def render_edit_todo_page(request: Request,todo_id:int, db: db_dependency):
    try:
        ...
        return templates_.TemplateResponse("edit-todo.html",{"request":request,"todo":todo, "user": user})
    except:
        return redirect_to_login()
...
async def read_by_id(user: user_dependency ,db: db_dependency, todo_id: int = Path(gt = 0)):
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    todo = db.query(Todo).filter(Todo.id == todo_id).filter(Todo.owner_id == user.get('id')).first()#first ilk gelen elemanı döndürecek
    if todo is not None:
        return todo
...
async def update(db:db_dependency ,user: user_dependency,
...
async def delete(user: user_dependency,db:db_dependency, todo_id: int = Path(gt=0)):
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    todo = db.query(Todo).filter(todo_id == Todo.id).filter(Todo.owner_id == user.get('id')).first()
    if todo is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
...
def create_todo_with_gemini(todo_string: str):
    ...
    [
            HumanMessage(
                content="I will provide you a todo item to add my to do list. What i want you to do is to create a longer and more comprehensive description of that  todo item, my next message will be my todo. Ve bütün bunları türkçe olarak cevapla."),
